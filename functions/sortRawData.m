...
function sortRawData(project, cohortPath)
    ...
    %% 1) Preparation
    % As long as your common_utils folder is parallell to the folder of this script, this will include that folder. Functions used in this file:
    % readBrukerParamFile.m
    root_dir                = fileparts(pwd);
    utils_path              = fullfile(root_dir, 'common_utils');
    addpath(utils_path);
            ...
            if isfile(acqpPath)
                ...
                for key = 1:length(keywords)
                    if contains(lower(scan_name), lower(keywords(key)))
                        ...
                        newPath = fullfile('R:\Henrik Elias', project, cohortPath, cohortStruct(subject).name, folderKey, scan_name);
                        %newPath = fullfile('R:\Preprocessed data from Paravision', project, cohortPath, cohortStruct(1).name, folderKey, scan_name);
...
close all
>>>>>>> 53660a0 (First local commit of rawToDICOM code)
end